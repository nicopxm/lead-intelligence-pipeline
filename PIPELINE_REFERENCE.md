...
- **`thresholds`** — `hot` (instant alert), `review` (normal CRM entry), `nurture` (below this, log-only — no draft email is generated, since drafting for a lead nobody will contact is a pure token-cost waste).
