...
**Char cap**: 4,000 characters per page after stripping `<script>`/`<style>`/`<nav>` and all remaining tags (docs/DECISIONS.md). A page whose stripped text is under 40 characters (e.g. a JS-only SPA that returns an almost-empty shell) is recorded as `status: "failed", reason: "empty_content"` even though the HTTP fetch itself succeeded — the fetch working isn't the same as getting usable content.
