...
**Known pre-existing regression, filed separately, not fixed in #23**: [#26](https://github.com/nicopxm/lead-intelligence-pipeline/issues/26) — `Tech Stack Detector` (#21) is currently failing every real lead with `tech_stack.status: "failed", reason: "config_unreadable"`, even though `/config/tech_fingerprints.json` is present, readable via `docker exec ... cat`, and a raw `fast-glob` call against the same path succeeds when run directly inside the container. Root cause not yet isolated (likely inside `Parse Config`'s `getBinaryDataBuffer` call, silently swallowed by its own try/catch) — out of scope for orchestration composition, tracked separately.
